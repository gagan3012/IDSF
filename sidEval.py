...
def getScores(predPath, goldPath):
    goldSlots, goldIntents = readNlu(goldPath)
    predSlots, predIntents = readNlu(predPath)
    ...
    results['f1'] = 0.0 if prec+rec == 0.0 else 2 * (prec * rec) / (prec + rec)
    print('recall:   ', rec)
    ...
    results['intents'] = corIntents/len(goldSlots)
    results['fullCor'] = fullyCor/len(goldSlots)
    ...
    f1 = 0.0 if prec+rec == 0.0 else 2 * (prec * rec) / (prec + rec)
    print('ul_slot-f1:  ', f1)
    results['ul_recall'] = rec
    results['ul_precision'] = prec
    results['ul_f1'] = f1
    ...
    f1 = 0.0 if prec+rec == 0.0 else 2 * (prec * rec) / (prec + rec)
    print('l_slot-f1:  ', f1)
    results['l_recall'] = rec
    results['l_precision'] = prec
    results['l_f1'] = f1
